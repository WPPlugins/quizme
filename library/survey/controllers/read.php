...
*****************************************************************************************/
class sv46v_read extends wv46v_action {
/*****************************************************************************************
* ??document??
*****************************************************************************************/
...
* ??document??
*****************************************************************************************/
	public function v46v_quizme_readWPfilter($return,$form) {
		if($this->application()->slug!='quizme')
		{
			return $return;
		}
		return $this->read ( $return,$form );
	}
	public function v46v_submission_readWPfilter($return,$form) {
	...
	public function v46v_surveyme_readWPfilter($return,$form) {
	...
	public function v46v_pollme_readWPfilter($return,$form) {
	...
	public function v46v_contactme_readWPfilter($return,$form) {
	...
	public function read($return,$form) {
			...
			if ($this->request()->is_post ($form)) {
				...
				**/
				...
				if ($return == '0') {
					switch ($field_definition ['type']) {
						case 'number_captcha' :
							if ($this->captchaEncode ( $_POST [$field] ['question'] ) != $_POST [$field] ['answer']) {
								$return = 1;
							}
							break;
						case 'recaptcha' :
...
*****************************************************************************************/
		...
		foreach ( $return['field_definitions'] as $key => &$value ) {
			if (isset ( $types [$value ['type']] )) {
				$value = bv46v_data_array::merge ( $types [$value ['type']], $value );
				if(null===$value['field'])
				{
					$value ['field'] = $cnt ++;
				}
...
*****************************************************************************************/
		...
		foreach ( $return['field_definitions'] as &$value ) {
			...
			{
				$value['options'] = apply_filters('sv46v_field_options',$value['options']);
			}
		}
		unset ( $value );
/*****************************************************************************************
