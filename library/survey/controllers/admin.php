...
* all the admin pages
*****************************************************************************************/
class sv46v_admin extends wv46v_action {
/*****************************************************************************************
* ??document??
*****************************************************************************************/
	...
	public function FormsEditAction() {
		$this->view->body = '';
		$this->view->general = $this->data ( $_GET ['edit'] )->post ( 'general' );
		$this->view->data_collection = $this->data ( $_GET ['edit'] )->post ( 'data_collection' );
		$this->view->graphics = $this->data ( $_GET ['edit'] )->post ( 'graphics',true );
		$this->view->field_definitions = $this->data ( $_GET ['edit'] )->post ( 'field_definitions' );
		$this->view->views = $this->data ( $_GET ['edit'] )->post ( 'views' );
		$return = '';
		$recaptcha = new wv46v_recaptcha ( $this->application () );
		$recaptcha_keys = $this->data ( $_GET ['edit'] )->post ( 'recaptcha_keys' );
		...
		$return = '';
		$this->view->body = '<ul class="v46v_field_definitions">';
		...
		foreach($this->view->field_definitions as $key=>$value){
			...
			$this->view->table_type='v46v_'.$value['type']."{$middle}";
			if($value['type']!='new')
			{
				{
...
//		$this->view->body .= '<li>'.$this->render_table().'</li>';
		...
		$this->view->table_type='v46v_general';
...
*****************************************************************************************/
	public function FormsActionMeta($return) {
		...
		$return ['classes'] [] = 'v46v_16x16_form';
		$return ['priority'] = - 1;
		return $return;
	}
