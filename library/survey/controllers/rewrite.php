...
* url rewriting
*****************************************************************************************/
class sv46v_rewrite extends wv46v_action {
/*****************************************************************************************
* ??document??
*****************************************************************************************/
...
*****************************************************************************************/
	private function png($form, $switch) {
		...
		if (isset ( $polls [$switch - 1] )) {
			...
			$sql = "SELECT `%s` as 'option',count(*) as 'count' from `%s` WHERE `%s` in(%s) AND (`info[submission]` is null or `info[submission]` = 1) GROUP BY `%s` ORDER BY `%s` DESC;";
